...
def coletar_metadados_raw():
    ...
    if not registros_para_adicionar and df_existente.empty:
        ...
        for arquivo in arquivos:
            ...
            num_registros = contar_linhas_csv(caminho_arquivo) if arquivo.lower().endswith(".csv") else contar_linhas_parquet(caminho_arquivo)
        ...
        registros_para_adicionar = registros_iniciais
    ...
    if registros_para_adicionar:
        ...
        registros_para_adicionar.append({
            ...
            "numero_registros": contar_linhas_csv(METADADOS_FILE),
...
def enviar_pasta_raw_para_minio(df_novos_metadados=None) -> bool:
    ...
    for _, row in df_novos_metadados.iterrows():
        arquivo = row["arquivo"]
        # Ignora apenas raw_metadados_simbr.csv
        if arquivo == "raw_metadados_simbr.csv":
            continue
        caminho_arquivo = os.path.join(RAW_DIR, arquivo)
        if not os.path.exists(caminho_arquivo):
            print(f"[WARN] Arquivo '{arquivo}' não encontrado, pulando...")
            continue
        try:
            s3_client.upload_file(caminho_arquivo, MINIO_BUCKET, arquivo)
            print(f"Arquivo '{arquivo}' enviado para MinIO no bucket '{MINIO_BUCKET}'.")
            enviados = True
